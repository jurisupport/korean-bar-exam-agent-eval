#!/bin/bash
# F1 경합문항 딥다이브 병렬 런처.
# 각 문항은 F1_voted.json과 픽스처의 격리 복제본에서 독립 실행하고,
# 결과 항목만 문항별 patch.json으로 추출한다. 공유 voted 병합은 별도 단계에서 한다.
set -euo pipefail

CR="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"
[ "$#" -ge 1 ] || { echo "사용: bash tools/x_run_deepdives.sh Q [Q ...]" >&2; exit 2; }
...
run_q() {
  ...
  cp "$CR/01_sealed/_parts/F1_voted.json" "$work/01_sealed/_parts/F1_voted.json"
  cp "$CR/tools/f_deepdive_prompt.md" "$work/tools/f_deepdive_prompt.md"

  sed -e 's/{RUN}/1/g' "$CR/tools/f_deepdive_prompt.md" > "$prompt"
  sed -e "s/{Q}/$q/g" "$CR/tools/f_deepdive_invocation.md" >> "$prompt"
  ...
  python3 "$CR/tools/x_extract_deepdive.py" \
    "$CR/01_sealed/_parts/F1_voted.json" \
    "$work/01_sealed/_parts/F1_voted.json" \

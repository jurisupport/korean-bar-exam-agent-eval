...
voted_path = os.path.join(cr, "01_sealed", "_parts", "F2_voted.json")
patches = sorted(glob.glob(os.path.join(cr, "01_sealed", "_parts", "F2_deepdive_q*.patch.json")))

with open(voted_path, encoding="utf-8") as f:
    original = json.load(f)
expected = sorted(x["no"] for x in original["answers"] if x.get("contested") is True)
assert expected, "F2_voted에 경합문항 없음"
...
for path in patches:
    ...
    q = patch["question"]
    seen.append(q)
    assert patch["run"] == 2 and patch["model"] == "gpt-5.6-sol"
...
print(f"✔ F2_voted 병합: 딥다이브 {len(expected)}/{len(expected)}, 비대상 문항 보존")
